...
static IWICImagingFactory* _GetWIC() noexcept
{
...
static inline void FitPowerOf2(UINT origx, UINT origy, UINT& targetx, UINT& targety, size_t maxsize)
{
    ...
    _Outptr_opt_ ID3D11ShaderResourceView** textureView) noexcept
{
    ...
    UINT twidth = width;
    UINT theight = height;
    if (loadFlags & (uint32_t)WIC_LOADER_FLAGS::WIC_LOADER_FIT_POW2)
    {
    ...
    if (loadFlags & (uint32_t)WIC_LOADER_FLAGS::WIC_LOADER_MAKE_SQUARE)
    {
    ...
    if (loadFlags & (uint32_t)WIC_LOADER_FLAGS::WIC_LOADER_FORCE_RGBA32)
    {
    ...
    // Handle sRGB formats
    if (loadFlags & (uint32_t)WIC_LOADER_FLAGS::WIC_LOADER_FORCE_SRGB)
    {
        format = MakeSRGB(format);
    }
    else if (!(loadFlags & (uint32_t)WIC_LOADER_FLAGS::WIC_LOADER_IGNORE_SRGB))
    {
        ...
        if (SUCCEEDED(frame->GetMetadataQueryReader(metareader.GetAddressOf())))
        {
            ...
            if (SUCCEEDED(metareader->GetContainerFormat(&containerFormat)))
            {
                ...
                if (memcmp(&containerFormat, &GUID_ContainerFormatPng, sizeof(GUID)) == 0)
                {
                    ...
                    {
                        sRGB = (loadFlags & (uint32_t)WIC_LOADER_FLAGS::WIC_LOADER_SRGB_DEFAULT) != 0;
                    }
                }
...
HRESULT TextureLoader::CreateWICTextureFromFileEx(
